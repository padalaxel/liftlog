...
export default function WorkoutDetailPage() {
  ...
  const title = workout?.program_days?.name ?? "Workout";

  return (
    <AppShell>
      <PageHeader title={title} backHref="/history" />
      <main className="flex flex-1 flex-col space-y-3 p-3">
            ...
            <section key={template.id} className="rounded border border-zinc-800 bg-zinc-900 p-3">
              ...
              <div className="space-y-2">
                {(setsByTemplate.get(template.id) ?? []).map((set) => (
                  <div key={set.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                    <span className="text-zinc-500">Set {set.set_number}</span>
                    <span className="text-zinc-400">
                      Plan {set.target_weight}×{set.target_reps}
                    </span>
                    <span className="text-zinc-200">
                      Logged {set.actual_weight ?? "—"}×{set.actual_reps ?? "—"}
                    </span>
        ...
        <section className="rounded-lg border border-zinc-800 bg-zinc-900 p-3">
          ...
          <div className="flex gap-2 pt-1">
            <input
              className="h-11 min-w-0 flex-1 rounded-md border border-zinc-700 bg-zinc-950 px-3 text-base"
              value={chatInput}
            ...
            />
            <button
              className="h-11 shrink-0 rounded-md bg-zinc-100 px-4 text-sm font-semibold text-zinc-900 disabled:opacity-50"
